...
// 复核枚举：review_status none/pending/approved/rejected（见 constants/enums.go）。
type ExamRecord struct {
	...
	GradedAt        *time.Time         `bson:"graded_at,omitempty" json:"graded_at"`   // 批改完成时间（复核窗口起点）
	...
	PassScore       float64            `bson:"pass_score" json:"pass_score"`   // 及格线快照（建记录时从试卷复制）
	...
	// 成绩复核闭环字段：申请一旦存在，普通批改通道关闭，分数只可由受理动作更正
	ReviewID       primitive.ObjectID `bson:"review_id,omitempty" json:"review_id,omitempty"`
	ReviewStatus   string             `bson:"review_status" json:"review_status"`     // none/pending/approved/rejected
	ScoreCorrected bool               `bson:"score_corrected" json:"score_corrected"` // 最终分是否经复核受理更正
}
